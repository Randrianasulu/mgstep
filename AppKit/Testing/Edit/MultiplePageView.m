...
- (unsigned) numberOfPages					{ return numPages; }
- (float) pageSeparatorHeight				{ return 5.0; }
...
- (NSColor *)lineColor						{ return lineColor; }
- (NSColor *)marginColor					{ return marginColor; }
...
- (void) drawRect:(NSRect)rect 
{
    if ([[NSGraphicsContext currentContext] isDrawingToScreen]) 
		{
        NSSize paperSize = [printInfo paperSize];
		float sh = [self pageSeparatorHeight];
        unsigned firstPage = rect.origin.y / (paperSize.height + sh);
        unsigned lastPage = NSMaxY(rect) / (paperSize.height + sh);
        unsigned cnt;
