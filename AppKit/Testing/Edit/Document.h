...
{
    NSTextView *_textView;		/* If nil, never saved */
...
+ (BOOL) openUntitled;								// Brings window front
					...
					// These set/get the documentName instance var and also set 
					// the window title accordingly. "nil" is used if no title.
...
- (BOOL) saveDocument:(BOOL)showSavePanel;	/* Saves under documentName; if not 
												set, tries to set it first */
- (BOOL) canCloseDocument;	/* Assures document is saved or user doesn't care 
							about the changes; returns NO if user cancels */
