...
static NSGlyphGenerator *__sharedGlyphGenerator = nil;
...
- (void) setAttachment:(NSTextAttachment *)anObject	{ _attachment = anObject; }
...
- (BOOL) trackMouse:(NSEvent *)event
		...
		untilMouseUp:(BOOL)flag						{ return YES; }
- (NSSize) cellSize									{ return NSZeroSize; }
...
{
	if (!__sharedGlyphGenerator)
		__sharedGlyphGenerator = [[self alloc] init];

	return __sharedGlyphGenerator;
}
...
static BOOL __NSShowGlyphBoxes = NO;
...
- (void) insertTextContainer:(NSTextContainer *)container atIndex:(NSUInteger)index;
{
	...
	_textContainerInfo=(struct _NSTextContainerInfo *) realloc(_textContainerInfo, sizeof(_textContainerInfo[0])*(cnt+1));	// (re)allocate memory
	if(index != cnt)
...
///				[ctxt _setTM:tm];
				...
				glyphs=malloc(sizeof(*glyphs)*(count+1));	// stores NSNullGlyph at end
						  ...
						  rectCount:(NSUInteger *) rectCount;
{
	...
	*rectCount=0;
		...
		if(i == *rectCount)
			{ // could not merge: create new rect
				if(*rectCount == _rectArrayCapacity)
					_rectArray=realloc(_rectArray, sizeof(_rectArray[0])*(_rectArrayCapacity=2*_rectArrayCapacity+5));	// increase with some safety margin
	...
	if(*rectCount+5 < _rectArrayCapacity/2)	// this time much smaller
		_rectArray=realloc(_rectArray, sizeof(_rectArray[0])*(_rectArrayCapacity=*rectCount+5));	// reduce with some safety margin
	return _rectArray;
}
...
- (void) setLayoutRect:(NSRect) rect
					 ...
					 characterIndex:(NSUInteger) attachmentIndex;
{
	...
	if([cell isKindOfClass:[NSTextAttachmentCell class]])
		...
		[(NSTextAttachmentCell *) cell drawWithFrame:rect
											  inView:[self firstTextView]];
#endif		// FIX ME no NSTextAttachmentCell
}
				 ...
				 containerOrigin:(NSPoint)containerOrigin;
{
	...
	[self drawStrikethroughForGlyphRange:glyphRange
					   ...
					   baselineOffset:[_typesetter baselineOffsetInLayoutManager:self glyphIndex:glyphRange.location]
...
- (id) initWithCoder:(NSCoder *) coder;
{
	...
	_textContainerInfo=calloc([_textContainers count], sizeof(_textContainerInfo[0]));
		...
		characterIndex:(NSUInteger) index;
{
	...
	if(!_glyphs || _numberOfGlyphs+length >= _glyphBufferCapacity)
		_glyphs=(struct NSGlyphStorage *) realloc(_glyphs, sizeof(_glyphs[0])*(_glyphBufferCapacity=_numberOfGlyphs+length+20));	// make more space
#if 0
...
{
	if(!g->extra)
		g->extra=(struct NSGlyphStorageExtra *) calloc(1, sizeof(g->extra[0]));
}
