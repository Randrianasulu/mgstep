...
static NSDictionary *__entitiesTable = nil;



@interface NSMutableString (NSXMLParser)
...
@implementation _NSMutableUString (NSXMLParser)
									  ...
									  range:(NSRange)searchRange
{
	...
	r = [self rangeOfString: replace options: opts range: searchRange];
	if (r.length > 0)
		{
		...
		do {
			count++;
			[(NSMutableString*)self replaceCharactersInRange: r withString: by];
			...
			if ((opts & NSBackwardsSearch) == NSBackwardsSearch)
				...
				newEnd = NSMaxRange(searchRange) + byLen - r.length;
				searchRange.location = r.location + byLen;
...
// currently used only by NSPropertyListSerialization - maybe NSXMLNode?
 ...
 */
	NSError *e=[NSError errorWithDomain:NSXMLParserErrorDomain
						code:err
						userInfo:[NSDictionary dictionaryWithObjectsAndKeys:msg, @"Message", nil]];
#if 0
	...
	if(!e /* acceptHTML */)
		...
		if(!__entitiesTable)
			{ // dynamically load entity translation table on first use
				...
				__entitiesTable = [d mutableCopy];
				NSAssert1(__entitiesTable, @"could not load and parse file %@", path);
#if 0
				NSLog(@"bundle=%@", b);
				NSLog(@"path=%@", path);
				NSLog(@"__entitiesTable=%@", __entitiesTable);
#endif
				[arp release];
			}
		e=[__entitiesTable objectForKey:entity];	// look up string in entity translation table
		}
...
{ // stall - i.e. queue up calls to delegate method
	...
	if(flag != isStalled)
		...
		if(!isStalled)
			...
			NSLog(@"stall cleared");
#endif
			[self performSelector:@selector(_parseData:)
				  withObject:[NSData data]
				  afterDelay:0.0];	// continue processing as soon as possible
			}
		}
}
